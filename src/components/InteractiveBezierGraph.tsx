...
function generateCurvePath(bezier: [number, number, number, number], w: number, h: number): string {
...
function generateSpeedPath(bezier: [number, number, number, number], w: number, h: number, speedH: number): string {
  const [x1, y1, x2, y2] = bezier;
  const innerW = w - PADDING * 2;
  const points: string[] = [];
  const step = 1 / 120;
...
function tryHaptic(style: "light" | "medium" | "heavy" = "light") {
  try {
    if (navigator.vibrate) {
      navigator.vibrate(style === "light" ? 5 : style === "medium" ? 10 : 20);
    }
  } catch {}
}
...
const InteractiveBezierGraph = ({
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (dragging === 1) {
      rawX = Math.max(0, Math.min(1, (pt.x - PADDING) / innerW));
      rawY = Math.max(-0.5, Math.min(2, (innerH - (pt.y - PADDING)) / innerH));
      newBezier[0] = snapEnabled ? snapValue(rawX, gridSnap) : rawX;
      newBezier[1] = snapEnabled ? snapValue(rawY, gridSnap) : rawY;
    } else {
      rawX = Math.max(0, Math.min(1, (pt.x - PADDING) / innerW));
      rawY = Math.max(-0.5, Math.min(2, (innerH - (pt.y - PADDING)) / innerH));
      newBezier[2] = snapEnabled ? snapValue(rawX, gridSnap) : rawX;
      newBezier[3] = snapEnabled ? snapValue(rawY, gridSnap) : rawY;
    }

    // Haptic on grid snap
    if (snapEnabled) {
      const snapKey = newBezier.map(v => v.toFixed(2)).join(",");
    ...
    onChange(newBezier);
  }, [dragging, onChange, bezier, getSVGPoint, innerW, innerH, snapEnabled, gridSnap]);

  const handlePointerUp = useCallback(() => {
    if (dragging) tryHaptic("light");
    setDragging(null);
  }, [dragging]);
  ...
  const snapDots = useMemo(() => {
    if (!snapEnabled || !interactive) return [];
    const dots: { x: number; y: number }[] = [];
    for (let gx = 0; gx <= 1; gx += 0.25) {
      for (let gy = 0; gy <= 1; gy += 0.25) {
        dots.push({
          x: gx * innerW + PADDING,
          y: innerH - gy * innerH + PADDING,
        });
      }
    }
    return dots;
  }, [snapEnabled, interactive, innerW, innerH]);
    ...
    <div className="ae-panel">
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((frac) => (
          <g key={`grid-${frac}`}>
            <line x1={PADDING} y1={PADDING + frac * innerH} x2={width - PADDING} y2={PADDING + frac * innerH} stroke="hsl(var(--ae-grid))" strokeWidth={frac === 0 || frac === 1 ? 1 : 0.5} />
            <line x1={PADDING + frac * innerW} y1={PADDING} x2={PADDING + frac * innerW} y2={height - PADDING} stroke="hsl(var(--ae-grid))" strokeWidth={frac === 0 || frac === 1 ? 1 : 0.5} />
          </g>
        ))}

        {/* Snap dots */}
        {snapDots.map((d, i) => (
          <circle key={`snap-${i}`} cx={d.x} cy={d.y} r={1.5} fill="hsl(var(--primary) / 0.3)" />
        ))}
        ...
        {/* Linear reference */}
        <line x1={startPt.x} y1={startPt.y} x2={endPt.x} y2={endPt.y} stroke="hsl(var(--muted-foreground) / 0.15)" strokeWidth={1} strokeDasharray="4 4" />

        {/* Target curve */}
        {targetPath && (
          <path d={targetPath} fill="none" stroke="hsl(var(--ae-green) / 0.4)" strokeWidth={2.5} strokeDasharray="6 4" />
        )}
        ...
        {interactive ? (
          <>
            ...
            <circle cx={cp1.x} cy={cp1.y} r={20} fill="transparent" onPointerDown={handlePointerDown(1)} className="cursor-grab active:cursor-grabbing" />
            <circle
              cx={cp1.x} cy={cp1.y} r={dragging === 1 ? 10 : 7}
              fill={`hsl(${color})`}
              stroke="hsl(var(--background))"
              strokeWidth={2}
              className="pointer-events-none"
              style={{ filter: dragging === 1 ? `drop-shadow(0 0 8px hsl(${color}))` : "none", transition: "r 0.15s" }}
            />
            <circle cx={cp2.x} cy={cp2.y} r={20} fill="transparent" onPointerDown={handlePointerDown(2)} className="cursor-grab active:cursor-grabbing" />
            <circle
              cx={cp2.x} cy={cp2.y} r={dragging === 2 ? 10 : 7}
              fill={`hsl(${color})`}
              stroke="hsl(var(--background))"
              strokeWidth={2}
              className="pointer-events-none"
              style={{ filter: dragging === 2 ? `drop-shadow(0 0 8px hsl(${color}))` : "none", transition: "r 0.15s" }}
            />
          </>
        ) : (
          <>
        ...
        {showSpeed && speedPath && (
          <>
            ...
            <path d={speedPath} fill="none" stroke="hsl(var(--ae-green))" strokeWidth={1.5} strokeLinecap="round" transform={`translate(0, ${height})`} />
            <path d={`${speedPath} L${width - PADDING},${speedH} L${PADDING},${speedH} Z`} fill="hsl(var(--ae-green) / 0.08)" transform={`translate(0, ${height})`} />
          </>
        )}
      </svg>
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-t border-border bg-card">
        ...
        {interactive && (
          <span className="ae-mono text-[9px] text-muted-foreground/50">
            {snapEnabled ? "snap: 0.05" : "drag handles"}
          </span>
        )}
      </div>
    </div>
